...
class AdminPanel:
    ...
    async def show_admin_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """عرض قائمة الادمن الرئيسية"""
        if not self.is_admin(update.effective_user.id):
            await update.message.reply_text("ليس لديك صلاحية الوصول!")
            return

        keyboard = [
            [InlineKeyboardButton("تعديل الاسعار", callback_data='admin_prices')],
            [InlineKeyboardButton("تعديل سعر الصرف", callback_data='admin_exchange')],
            [InlineKeyboardButton("عرض الشكاوي", callback_data='admin_complaints')],
            [InlineKeyboardButton("احصائيات", callback_data='admin_stats')],
            [InlineKeyboardButton("اغلاق", callback_data='close_menu')]
        ]
...
"
    ...
    async def show_prices_editor(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            text_lines = ["تعديل اسعار المحروقات:
"]
            keyboard = []
...
".join(text_lines)
            ...
            await context.bot.send_message(
                chat_id=target,
                text="حدث خطأ اثناء تحميل قائمة الاسعار. الرجاء التحقق من السجلات."
            )
    ...
    async def handle_price_edit(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not price_id:
                if query:
                    await query.answer("لا استطيع قراءة معرف السعر.", show_alert=True)
                return
...
"
                f"ارسل السعر بالليرة السورية (القديمة).
"
                f"سيتم حساب السعر الجديد تلقائيا (قسمة على 100).
"
                f"مثال: `8500`"
            )
            if query:
                await query.edit_message_text(prompt_text, parse_mode='Markdown',
                                              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("الغاء", callback_data='admin_menu')]]))
            else:
            ...
            await context.bot.send_message(chat_id=Config.ADMIN_ID, text="حدث خطأ اثناء بدء تعديل السعر.")

    async def show_complaints(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """عرض قائمة الشكاوي للادمن"""
        query = getattr(update, "callback_query", None)
        if query:
            try:
                await query.answer()
...
النص: {c.complaint_text}"""
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="انقر على زر لتحديث حالة اي شكوى",
                reply_markup=reply_markup
            )
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="حدث خطأ اثناء تحميل الشكاوي."
            )
