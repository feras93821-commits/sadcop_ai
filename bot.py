...
"
            f"رقم الشكاوي: {Config.COMPLAINT_PHONE}

"
            f"الآن يرجى إرسال رقم هاتفك للتواصل معك (او اكتب 'تخطي'):",
            parse_mode='Markdown'
        )
        return
...
"
            "يمكنني مساعدتك في:
"
            "- معرفة اسعار المحروقات (بنزين، مازوت، غاز...)
"
            "- تقديم شكوى او اقتراح
"
            "- الرد على استفساراتك العامة

"
            "جرب ان تسألني مثلاً: *كم سعر البنزين؟* او *أريد تقديم شكوى*",
            parse_mode='Markdown'
        )

async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != Config.ADMIN_ID:
        await update.message.reply_text("هذا الامر مخصص للمدير فقط.")
        return
...
"
                "ارسل السعر الجديد (1 دولار = كم ليرة؟)
"
                "مثال: `15000`",
                parse_mode='Markdown',
            ...
            stats = f"""احصائيات البوت

انواع الوقود: {len(prices)}
اجمالي الشكاوي: {len(complaints)}
...
"""
            for p in prices:
                stats += f"
- {p.fuel_type}: {p.price_syp:,.0f} ل.س (قديم) / {p.price_syp_new:,.2f} ل.س (جديد) / {p.price_usd} $"
...
async def admin_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = """اوامر لوحة تحكم الادمن:
...
"
            ...
            await update.message.reply_text("نوع الوقود غير موجود! الانواع المتاحة: بنزين، مازوت، غاز منزلي، غاز صناعي")
...
async def admin_complaints(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not complaints:
        await update.message.reply_text("لا توجد شكاوي حالياً.")
        return
